...
const Hero = ({ cityData }: { cityData: PlaceDetails }) => {
  ...
  return (
    <section className="bg-[#081933]/90 relative  2xl:flex 2xl:justify-center">
      ...
      {/* Breadcrumb for better seo and accessibility */}
      <div className="max-w-7xl mx-auto mb-6 text-sm text-white absolute top-[110px] md:top-[170px] left-[20px] md:left-[50px]">
        <Link href="/" className="hover:text-slate-300">
          Home
        </Link>
        <ChevronRight className="inline-block w-4 h-4 mx-1" />
        <Link href="/available-areas" className="hover:text-slate-300">
          Locations
        </Link>
        <ChevronRight className="inline-block w-4 h-4 mx-1" />
        <span className="text-slate-200 font-medium">{cityData.name}</span>
      </div>
      {/* main contenr of hero section */}
      <div className="overflow-hidden flex max-md:gap-6 max-md:flex-col md:items-start md:pl-8 lg:pl-12 pb-6 pt-32 md:pt-52 xl:pt-48">
        <div
          className={`max-md:self-center max-md:text-center ${
            isMounted ? "translate-y-0" : "-translate-y-20 opacity-0"
          } transition-all duration-1000 pt-6 md:pt-4`}
        >
          <h1 className="font-bold max-w-[630px] tracking-wide scale-y-[1.2] md:scale-y-[1.1] md:leading-[1] leading-[38px] text-[34px] md:text-4xl lg:text-[50px] xl:text-[60px] text-emerald-50">
            Fast & Reliable 24/7 <span className="text-red-500">Ambulance</span>{" "}
            Service in <span className="text-blue-500">{cityData.name}</span>
          </h1>
        ...
        <div>
          <Image
            ...
            className={`${
              isMounted
                ? "-translate-x-4 md:translate-x-0 md:scale-[1.16] opacity-100"
                : "translate-x-44 scale-50 opacity-0"
            } transition-all duration-1000 overflow-visible h-full w-full object-cover`}
          />
        </div>
      </div>
    </section>
